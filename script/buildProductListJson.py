...
for category_name, products in productList.items():
    for product_name, info in products.items():
        ...
        if checkPageExists(imagePath, pid) is not None:
            procceededProductList[pid]['image'] = f'/assets/newestBiotech/images/products/products_display/{checkPageExists(imagePath, pid)}'
        else:
        ...
        if checkPageExists(iConPath, pid) is not None:
            procceededProductList[pid]['overlay'] = f'/assets/newestBiotech/images/product-icon/products_display/{checkPageExists(iConPath, pid)}'
        else:
...
with open('error_list.json', 'w') as f:
    errorList['missImage'].sort()
    errorList['missIcon'].sort()
